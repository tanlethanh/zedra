...
usage() {
  echo "Usage: $0 --instance <instance[,instance,...]> [--service relay|monitor]"
  echo ""
  echo "Options:"
...
deploy_one() {
  ...
  if [[ "$SERVICE" == "relay" ]]; then
    docker save "zedra-relay:latest" | gzip | ssh "$ssh_host" 'docker load'
  elif [[ "$SERVICE" == "monitor" ]]; then
    docker save "zedra-monitor:latest" | gzip | ssh "$ssh_host" 'docker load'
  else
...
if [[ ${#INSTANCES[@]} -eq 1 && "${INSTANCES[0]}" == "local" ]]; then
  ...
  for instance in "${INSTANCES[@]}"; do
    (
      set -o pipefail
      deploy_one "$instance" 2>&1 | awk -v p="[${instance}]" '{ print p " " $0; fflush() }'
    ) &
    PIDS+=($!)
  done
