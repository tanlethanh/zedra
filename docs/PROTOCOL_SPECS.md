...
**Terminology:** A *managed agent* is one of `AgentKind`: Claude Code (`Claude`), Codex (`Codex`), OpenCode (`OpenCode`), Pi (`Pi`), Hermes (`Hermes`), and Maki (`Maki`). This is narrower than `AgentInstalledList`, which lists every terminal agent CLI the host can launch (Amp, Cline, Cursor, etc.). Most agents scope sessions per workspace; `Hermes` is a global personal agent whose sessions are workspace-independent.
...
- `AgentSummary.account.fields` exposes locally discovered account/setup metadata for manage-agent detail views. Values must remain privacy-safe. Current fields per agent kind:
  ...
  - **Maki**: Config presence (from `$XDG_CONFIG_HOME/maki` or `~/.config/maki`, legacy `~/.maki`) and Default model (from `config.toml` `[provider] default_model`). Sessions are workspace-scoped: scanned from `$XDG_STATE_HOME/maki/sessions/*.jsonl` (legacy `~/.maki/sessions`), filtered by each transcript's header `cwd`; the freshest `title`/`updated_at` come from the last `meta` record. Resume uses `maki --resume <id>` (`--session` alias). Maki has no external lifecycle-hook mechanism, so Zedra installs no hooks and emits no session-stop notifications; live binding falls back to terminal command detection (`maki` as first token).
