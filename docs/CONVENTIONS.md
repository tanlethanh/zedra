...
- **Native bridge** (alerts, sheets, action rows): pass the bare slug with `.icon("google")` on `AlertButton` and friends. iOS resolves it via the asset-catalog imageset and Android via the generated drawable, both named by slug.
