...
/opt/homebrew/bin/idevicesyslog | grep -E 'Zedra|zedra|panic|error' --line-buffered
```
...
ZedraFFI.xcframework/   # Built xcframework (committed); contains libzedra.a for
                        # device (ios-arm64) and simulator (ios-arm64-simulator)
...
crates/zedra/src/ (Rust)
  ...
  target/aarch64-apple-ios-sim/release/libzedra.a    (simulator)
        ...
        │
  ZedraFFI.xcframework/
    ios-arm64/libzedra.a                              (device slice)
    ios-arm64-simulator/libzedra.a                    (simulator slice)
        │
...
```yaml
- framework: ../ZedraFFI.xcframework # Rust static library
- sdk: Metal.framework
...
bridge().density()            → 3.0 (hardcoded)
bridge().system_inset_top()   → 59 (hardcoded, notch)
```
...
Rust log messages (via `oslog` crate) and NSLog both go to the device system log:

```bash
/opt/homebrew/bin/idevicesyslog | grep -E 'Zedra|zedra|panic|fault' --line-buffered
```

The `oslog` crate uses the subsystem `dev.zedra.app`.
...
The xcframework is committed to the repo. If you change Rust code that affects
the iOS build and only run `xcodebuild` (not `./scripts/build-ios.sh`), Xcode
uses the old `.a`. Always run `./scripts/build-ios.sh` after Rust changes.
