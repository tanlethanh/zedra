...
// Highlight queries embedded from vendor/zed's language definitions.
...
impl Language {
    ...
    /// Get tree-sitter language and highlight query for this language.
...
impl Highlighter {
    ...
    pub fn new(language: Language) -> Self {
        match language.grammar_and_query() {
            Some((ts_lang, query_source)) => {
                ...
                // Try to parse the query; fall back to no highlighting on error
    ...
    /// Create a highlighter configured for Rust syntax (convenience method).
    pub fn rust() -> Self {
    ...
    /// Get the language this highlighter is configured for.
